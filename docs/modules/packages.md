...
| `display` | 最长 `128` 个字符 | 缺省时使用 ID；当前 GUI 使用纯文本名称，不渲染 `&` 颜色代码或文本占位符。 |
| `description` | 字符串数组，最多 `32` 行 | 当前会保存到定义中，但现有列表 GUI 尚未显示 Description 或 Lore。 |
...
创建实例时会保存实例 UUID、所有者 UUID、礼包 ID、礼包版本、展示名称、图标、模式、物品原型快照、数量、来源事件、状态、发放时间和随机选择索引。当前实现没有单独的 `opened_at` 字段，也没有 `EXPIRED` 状态；不要在运维脚本中假定这些字段存在。
...
配置重载后，旧实例仍使用原快照；修改礼包定义不会改变玩家尚未打开的礼包。其中 `display` 和 `icon` 也会随实例快照保存，因此旧实例不会跟随新配置改变外观。
...
```text
PENDING -> OPENING -> OPENED
                  \\-> WAITING_INBOX
OPENING / DELIVERING / BLOCKED -> 人工检查
```

当前打开流程会写入 `OPENING`，随后直接写入 `OPENED` 或 `WAITING_INBOX`；`DELIVERING` 是为后续逐堆持久化投递保留的状态，当前版本不会主动写入它。服务端在打开时重新验证实例所有权和当前状态。可完整放入背包的堆叠立即投递；背包不足时，未成功投递的堆叠保留在实例中并进入 `WAITING_INBOX`，后续再次打开礼包列表即可继续投递。已成功投递的堆叠不会再次发放。已 `OPENED` 的实例不能重复打开。

随机礼包的选择索引在实例中持久化，因此断线或重启后不会重新随机。若实例处于 `OPENING`、`DELIVERING` 或 `BLOCKED`，服务端不会自动危险重放；应先备份世界数据并由管理员检查实例和日志。

当前 `PackageDeliveryBatch` 只是内存中的投递结果记录（已投递堆叠、待投递堆叠和完成标志），尚未作为独立 SavedData 持久化批次。实际恢复边界以实例中的 `items`、`quantities` 和状态为准；不要把 `PackageDeliveryBatch` 当作可供管理员查询的审计账本。
...
当前列表是 54 格容器，前 45 格最多显示 45 个未开启实例，未提供翻页。物品名称只显示实例快照的 `displayName`；不会显示配置中的 Description、来源、状态或物品预览。`give` 的 `amount` 范围为 `1`--`4096`，按玩家、按数量顺序创建；当前没有容量预检和整体事务，达到实例上限时此前已经创建的实例不会回滚。
...
当前版本明确不提供：独立确认 GUI、按权重随机、商店礼包商品及购买事务恢复、奖励箱独立礼包入口、指定实例玩家命令、`retry`/`resolve` 人工处理命令、独立持久化投递批次、完整批次审计界面、礼包占位符和实体礼包物品交易。后续若增加这些能力，应保持现有虚拟实例和快照模型，避免引入可复制的实体礼包。
