...
格式版本固定为 `3`。新安装请使用下面的推荐格式；旧服迁移时保留原文件备份并阅读[升级指南](../guides/upgrade-guide.md)。
...
| 字段 | 类型与范围 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `format_version` | 整数，必须为 3 | 3 | 根格式版本。 |
...
旧格式兼容：升级器会将旧根配置迁移到 v3 并创建备份。旧服已迁移出的 `allowed_roots: ["*"]` 保留可用性但会记录宽松模式警告；新服不要复制它。
