...
`custom` 原版统计不会在成就 lore 中显示内部资源 ID（例如 `minecraft:walk_one_cm`）；成就标题和描述仍然保留，避免把实现细节暴露给玩家。距离统计的界面进度统一以米显示，例如 `at_least: 10, unit: "kilometers"` 会显示为 `当前进度/10000 米`，但解锁阈值仍按配置单位精确计算。称号奖励会从称号 ID 解析为标题配置中的 `display`，并在其下方显示配置的 `tooltip` 和 `effects` 对应的效果展示文本；找不到对应称号或效果定义时不显示原始 ID。
