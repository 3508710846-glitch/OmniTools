...
abstract class AttachmentTargetsMixin implements AttachmentTargetImpl {
	...
	public void fabric_readAttachmentsFromNbt(class_11368 view) {
		...
		if (this.fabric_shouldTryToSync() && this.dataAttachments != null) {
			this.dataAttachments.forEach((type, value) -> {
				if (type.isSynced()) {
					acknowledgeSynced(type, value, view.method_71414());
				}
			});
		}
	}
	...
	private void acknowledgeSynced(AttachmentType<?> type, Object value, class_7225.class_7874 wrapperLookup) {
		class_5455 dynamicRegistryManager = (wrapperLookup instanceof class_5455 drm) ? drm : fabric_getDynamicRegistryManager();
